...
import { asArray, cloneRepo, findPackageJsonFiles } from "./utils.js";
...
for (const pkg of await fs.readdir("pkgs/plugins")) {
	...
	if (cache.success) {
		...
		const firstCommit = (await $$`git rev-list --max-parents=0 HEAD`)
			...
			.trim();
		console.info(
			`Checking out the first commit: '${firstCommit}' (No cache)`,
		);
	...
	const allCommits = Object.values(packageVersions).flatMap((pkg) =>
		Object.values(pkg),
	);
	...
	if (process.env.CRAWL_SECRET) {
		for (const pkg of plugin.packages) {
			...
			const pkgVersions = Object.entries(versions).map(
				([version, commit]) => ({
					version,
