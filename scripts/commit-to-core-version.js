...
import { asArray, findCargoLockFiles } from "./utils.js";
...
async function getCoreVersion(repoDir, commit) {
	...
	try {
		...
		cargoLock =
			await $$`git show ${commit}:${relativePathToCargoLock}`.text();
	} catch (ignored) {
		try {
		...
		const swcCoreVersions = await Promise.all(
			cargoLockFiles.map(async (file) => {
		...
		console.log(
			`Found multiple versions for commit ${commit}: ${versions.join(", ")}`,
		);
		return null;
	}
